...
class _UserDashboardState extends State<UserDashboard> {
  ...
  final String _apiUrl = "${Apis.BaseUrl}/SocialMediaApis/updateUserProfile.php";

  // In-memory cache for posts
  static List<dynamic> _cachedPosts = [];
  static int _cachedCurrentPage = 1;
  static bool _cachedHasMore = true;

  List<dynamic> _posts = [];
  int _currentPage = 1;
  final int _limit = 5; // Reduced limit to load fewer posts initially
  ...
  Future<void> _loadInitialData() async {
    if (_cachedPosts.isNotEmpty) {
      setState(() {
        _posts = List.from(_cachedPosts);
        _currentPage = _cachedCurrentPage;
        _hasMore = _cachedHasMore;
      });
    } else {
      await _fetchAndCachePosts();
    }
  }
  ...
  Future<void> _fetchAndCachePosts() async {
    ...
    try {
      ...
      print('Fetched ${newPosts.length} posts for page $_currentPage');

      // Fetch pagination metadata separately
      final response = await http.post(
        Uri.parse("${Apis.BaseUrl}SocialMediaApis/getImagePost.php"),
        headers: {"Content-Type": "application/x-www-form-urlencoded"},
        body: {
      ...
      setState(() {
        ...
        _cachedPosts = List.from(_posts);
        _currentPage++;
        _cachedCurrentPage = _currentPage;
        _hasMore = _currentPage <= totalPages && newPosts.isNotEmpty;
        _cachedHasMore = _hasMore;
        print('Updated posts count: ${_posts.length}, hasMore: $_hasMore, totalPages: $totalPages');
      });
    } catch (e) {
  ...
  Future<void> _shareToWhatsApp(int postIndex, BuildContext context) async {
    ...
    final String message = 'Check out this awesome post on Social Greet!\n$postUrl';
  ...
  Future<void> pickVideo(BuildContext context) async {
    ...
    if (pickedFile != null) {
      setState(() {
        ...
        context.read<UploadVideoPostBloc>().add(UploadVideoPostEvent(isUploaded: true));
      });
    }
  }
  ...
  Future<void> pickImage(BuildContext context) async {
    ...
    if (pickedFile != null) {
      setState(() {
        ...
        context.read<UploadImagePostBloc>().add(UploadImagePostEvent(isUploaded: true));
      });
    }
  }
                          ...
                          child: Container(
                            ...
                            child: ClipRRect(
                              ...
                              child: CachedNetworkImage(
                                imageUrl: _userProfilePics['user_$index'] ?? '',
                                fit: BoxFit.cover,
                ...
                child: _posts.isEmpty && _isFetching
                    ...
                    : _posts.isEmpty
                        ...
                        : ListView.builder(
                            ...
                            cacheExtent: MediaQuery.of(context).size.height * 2, // Increased cache extent
                            addAutomaticKeepAlives: true,
                            itemCount: _posts.length + (_hasMore ? 1 : 0),
                            itemBuilder: (context, index) {
                              if (index == _posts.length) {
                                if (!_isFetching) _fetchMorePosts();
                              ...
                              return VisibilityDetector(
                                ...
                                onVisibilityChanged: (info) {
                                  if (info.visibleFraction > 0.5) {
                                    _fetchUserProfilePic(_posts[index]['phone']);
                                  }
                                },
  ...
  Widget _buildPostItem(BuildContext context, int index, List<dynamic> posts) {
    ...
    final String mediaUrl = Apis.BaseUrl + "SocialMediaApis/" + post['postPath'];
                ...
                child: Stack(
                  ...
                  children: [
                    VisibilityDetector(
                      ...
                      onVisibilityChanged: (info) {
                        if (isVideo) {
                          if (info.visibleFraction > 0.5 && !_videoControllers.containsKey(mediaUrl)) {
                          ...
                          } else if (info.visibleFraction <= 0.5 && _videoControllers.containsKey(mediaUrl)) {
                            _videoControllers[mediaUrl]?.pause();
                            _videoControllers[mediaUrl]?.dispose();
                            _videoControllers.remove(mediaUrl);
                          }
                        }
                      },
                      child: isVideo
                          ? (_videoControllers.containsKey(mediaUrl) &&
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                          ...
                          ShaderMask(
                            ...
                            child: IconButton(
                              ...
                              onPressed: () => _showCommentsBottomSheet(
                                  context, index, post['id'], post['postPath'], post['postType'], post['phone']),
                            ),
                          ),
                          ShaderMask(
                            shaderCallback: (bounds) => LinearGradient(
                      ...
                      StreamBuilder<DatabaseEvent>(
                        stream: _database.child(post['id'].toString()).child('likeCount').onValue,
                  ...
                  SizedBox(height: MediaQuery.of(context).size.height * 0.01),
                  Text(
                    post['content'] ?? 'Dropping heat on Social Greet! #Colossal',
                    style: TextStyle(
                        fontSize: MediaQuery.of(context).size.width * 0.04, color: Colors.black87),
                  ),
                ],
              ),
            ),
  ...
  void _showCommentsBottomSheet(BuildContext context, int postIndex, int postId, String postPath,
      String postType, String phone) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
            ...
            BlocBuilder<UploadImagePostBloc, UploadImagePostState>(
              builder: (context, state) => ListTile(
                ...
                trailing: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    if (state.isUploaded) const Icon(Icons.verified, color: Colors.blue),
                    IconButton(
              ...
              builder: (context, state) => ListTile(
                ...
                trailing: Row(
                  ...
                  children: [
                    ...
                    IconButton(
                        onPressed: () =>
                            createTextPost(context, titleController.text, messageController.text),
                        icon: const Icon(Icons.upload)),
                  ],
                ),
            ...
            children: [
              ...
              Expanded(
                child: TextField(
                  ...
                  decoration:
                      const InputDecoration(labelText: 'Message', border: OutlineInputBorder()),
                ),
              ),
              SizedBox(height: MediaQuery.of(context).size.height * 0.02),
              Row(
  ...
  Future<void> createImagePost(BuildContext context, File? imageFile) async {
    if (imageFile != null) print('Uploading image: ${imageFile.path}');
  }

  Future<void> createVideoPost(BuildContext context, File? videoFile) async {
    if (videoFile != null) print('Uploading video: ${videoFile.path}');
  }

  Future<void> createTextPost(BuildContext context, String title, String message) async {
    print('Creating text post: $title - $message');
  }
}

// Transparent placeholder image for FadeInImage
          ...
          Container(
            ...
            child: Row(
              children: [
                CircleAvatar(
                  radius: 18,
                  backgroundImage: widget.userProfilePics[UserData.phone] != null
          ...
          child: Row(
            ...
            children: [
              CircleAvatar(
                ...
                backgroundColor: Colors.grey[200],
                child: profilePicUrl == null ? Icon(Icons.person, size: 25) : null,
              ),
              SizedBox(width: 12),
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 4),
                    Text(comment, style: TextStyle(fontSize: 14, color: Colors.black87)),
                  ],
                ),
              ),
              IconButton(
                icon: Icon(Icons.favorite_border, color: Colors.grey[600], size: 22),
                onPressed: () {},
              ),
            ],
          ),
