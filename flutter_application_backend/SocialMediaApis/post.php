...
if ($POST_TYPE == "video" && $POST_FILE) {
    ...
    if (move_uploaded_file($POST_FILE['tmp_name'], $targetFilePath)) {
        $query = "INSERT INTO dbo.Postmaster (postType, postPath, phone, userName, content) VALUES (?, ?, ?,?, ?)"; // Added userName
        $params = array($POST_TYPE, $targetFilePath, $MOBILE, $USERNAME,$POST_CONTENT); // Added USERNAME to params
