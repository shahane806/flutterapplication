...
if ($POST_TYPE == "video" && $POST_FILE) {
    ...
    if (move_uploaded_file($POST_FILE['tmp_name'], $targetFilePath)) {
        $query = "INSERT INTO dbo.Postmaster (postType, postPath, phone) VALUES (?, ?, ?)";
        $params = array($POST_TYPE, $targetFilePath, $MOBILE);
    ...
    $query = "INSERT INTO [dbo].[TextPost] (postType, phone, title, message)  
    VALUES  
    (?, ?, ?, ?)";
    
    $params = array($POST_TYPE, $MOBILE , $POST_TITLE, $POST_MESSAGE);
    $result = sqlsrv_query($conn, $query, $params);
...
elseif ($POST_TYPE == "liked") {
    $query = "INSERT INTO dbo.LikeMaster (postType, postPath, phone) VALUES (?, ?, ?)";
    $params = array($POST_TYPE, $POST_PATH, $MOBILE);
    $result = sqlsrv_query($conn, $query, $params);
    ...
    echo json_encode(["status" => 200, "message" => "Like added successfully", "file_path" => $POST_PATH]);
} elseif ($POST_TYPE == "bookedMarked") {
    $query = "INSERT INTO dbo.BookedMarkMaster (postType, postPath, phone) VALUES (?, ?, ?)";
    $params = array($POST_TYPE, $POST_PATH, $MOBILE);
    $result = sqlsrv_query($conn, $query, $params);
    ...
    $query = "INSERT INTO dbo.CommentMaster (postType, postPath, phone, comment) VALUES (?, ?, ?, ?)";
    $params = array($POST_TYPE, $POST_PATH, $MOBILE, $POST_COMMENT);
    $result = sqlsrv_query($conn, $query, $params);
    ...
    $POST_ID = $_POST['POST_ID'] ?? null;
    $query = "DELETE FROM dbo.BookedMarkMaster WHERE postId = ?";
    $params = array($POST_ID);
    $result = sqlsrv_query($conn, $query, $params);
    ...
    if ($result === false) {
        echo json_encode(["status" => 500, "message" => "Failed to remove like from LikeMaster", "error" => sqlsrv_errors()]);
        exit;
    }

    $rows_affected = sqlsrv_rows_affected($result);
    if ($rows_affected > 0) {
        echo json_encode(["status" => 200, "message" => "Like removed successfully from LikeMaster"]);
    } else {
        echo json_encode(["status" => 404, "message" => "No like record found for this post"]);
    }
} else {
